...
static const char char_path[] = "/dev/xil_xvc/cfg_ioc0";
...
static int getXVCProps(int fd, struct xil_xvc_properties *xvc_props){

	int ret = ioctl(fd, XDMA_RDXVC_PROPS, xvc_props);
	if (ret < 0)
	{
		printf("Could not run getXVCProps\n");
		printf("Error: %s\n", strerror(errno));
		return -1;
	} else {
		printf("Printing read xvc properties\n");
		printf(" XVC algo type - %02X\n",xvc_props->xvc_algo_type);
		printf(" Config vsec id - %04X\n",xvc_props->config_vsec_id);
		printf(" config_vsec_rev - %04X\n",xvc_props->config_vsec_rev);
		printf(" bar_index - %04X\n",xvc_props->bar_index);
		printf(" bar_offset - %04X\n",xvc_props->bar_offset);
	}
...
int main(int argc, char **argv)
{
	...
	struct xil_xvc_properties xvc_props;
	unsigned max_nbytes;
	...
	printf("XVC PCIE Driver Verified Successfully!\n");


	// Get xvc properties
	getXVCProps(fd, &xvc_props);

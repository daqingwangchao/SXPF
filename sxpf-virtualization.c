...
//all the virtual divice informations which are used for channal, port mapping and the device node pointer, inode and so on. 
virtual_dev_sxpf_t SXPFv_Info;
...
/**
 * convert param with int type from 0 to 1 or 1 to 0.
 * @param port: the address of to be transformed number(0..1)
*/
...
/**
 * From the virtual device ID get the Minor number of this device, the value is from 0 to 15, in the case of this number 
 * greter than 7, through this function then it will return to the circle 0..7 again.
*/
int MinorDevID_To_ValidChannelNr(int *minornumber) {
  
      u32 ch = MINOR(*minornumber);
      if (ch > 7) { // hier 7 can be in the further repalced by SXPFv_Info.fhl->card->nr to get the actual opened device nummer
        return ch%8;
      } else {
        return ch;
      }
...
/**
 * General mapping fucntion for chnanel and port
 * @param mode: chose one mapping mode
 * @param arg: address from user space
*/
static int sxpf_mapping_option(Mapping_Option_Def mode, unsigned long arg) {

   SXPFv_Info.Map_Opt = mode;
   u32 port = 0; // the following all the conditions in "if" are about the value when potr is equal to 0 

  if (SXPFv_Info.Map_Opt == PORT_WRREG_MAPPING) {

    Triggle(&port); //map port from 0 to 1
    pr_info("SXPFv: modified %d", port);
    ...
    if (rw.offs == 0x30 && rw.data == 0x4) { //for original_port 0, event 0
    ...
    else if (rw.bar == PLASMA_REGION && (rw.data == 6 || rw.data == 2)) { // 2 is from read_register
        rw.offs = 0x20 + 4 * port;
    }


    else if (rw.data == (0x0000000c|0x004e0000)) { //0xc is coded value from register_read; 
      ...
      pr_alert("SXPFv: ioctl_write_reg_v: unexpected value of rw.data from userspace\n");
    }
    ...
    Triggle(&port); //restore the port number for other loop usessage(default is 0)
    pr_info("write_reg::rw.offs: %d, rw.data: %d\n", rw.offs, rw.data);
    return 0;

  } else if (SXPFv_Info.Map_Opt == PORT_RDREG_MAPPING) {
      ...
      if (rw.bar == PLASMA_REGION) {
          Triggle(&port); //map port from 0 to 1
          rw.offs = 0x20 + 4 * port;
          Triggle(&port);//restore the port to 0
      } 

     else if (rw.bar == 0 && rw.offs == 0x90 + 0x20 * port) {
          Triggle(&port);//1
          rw.offs = 0x90 + 0x20 * port;
          Triggle(&port);//0
          ...
          pr_info("SXPFv: ioctl_read_reg_v: unexpected data to read register from user space!\n");
      }
    ...
    return 0;
  ...
  } else if (SXPFv_Info.Map_Opt == PORT_CMD_SYNC_MAPPING) {
      ...
      else if (request.cmd_stat == SXPF_CMD_I2C_SET_BAUDRATE | SXPF_CMD_STATUS_REQUEST) {
        request.args[3] = port; 
      }

      else {pr_info("SXPFv: unexpected data to SYCN from user space!\n");}
  ...
  } else if (SXPFv_Info.Map_Opt == CHANNEL_MAPPING) {

      u32 ch = MinorDevID_To_ValidChannelNr(&SXPFv_Info.current_opened_sxpfvID);
...
static int ioctl_relase_frame_v(unsigned long arg) {
    ...
    Mapping_Option_Def mode = CHANNEL_MAPPING;
    int ret;
    
    ret = sxpf_mapping_option(mode, arg);
    if (ret) {
      return ret;
    }
...
/**
 * Callback function from sxpfv_ioctl which is used for port mapping, default port is 0, if some conditions satisfied then it will be mapped to 1 otherwise it maintains 0;
 *
 * @param arg: address from user space
 * @return 0 sucess, other value fails.
*/
static int ioctl_write_reg_v(unsigned long arg){
    ...
    Mapping_Option_Def mode = PORT_WRREG_MAPPING;
    u32 port = 0;
    sxpf_rw_register_t  rw;

    if ((MinorDevID_To_ValidChannelNr(&SXPFv_Info.current_opened_sxpfvID) == 2 ||
        MinorDevID_To_ValidChannelNr(&SXPFv_Info.current_opened_sxpfvID) == 3) &&
        SXPFv_Info.cuurent_opened_real_sxpf_Nr == 1) {

        pr_info("SXPFv: now, i am in ioctl_write_reg_v, port: %d\n", port);
        int ret;
        ret = sxpf_mapping_option(mode, arg);
        if (ret) {
          return ret;
    ...
    } else{ pr_info("SXPFv: now, else ioctl_write_reg_v, port: %d\n", port); } // for case port is equal to 0 and 1
...
static int ioctl_read_reg_v(unsigned long arg) {
    ...
    Mapping_Option_Def mode = PORT_RDREG_MAPPING;
    sxpf_rw_register_t  rw;
    ...
    if ((MinorDevID_To_ValidChannelNr(&SXPFv_Info.current_opened_sxpfvID) == 2 ||
        ...
        pr_info("SXPFv: now, i am in ioctl_read_reg_v");
      
        int ret;
        ret = sxpf_mapping_option(mode, arg);
        if (ret) {
          return ret;
        } else{ pr_info("SXPFv: now, in read else"); }
...
static int ioctl_send_cmd_sync_v(unsigned long arg) {
    ...
    Mapping_Option_Def mode = PORT_CMD_SYNC_MAPPING;
    sxpf_cmd_fifo_el_t  request;
   
    if ((MinorDevID_To_ValidChannelNr(&SXPFv_Info.current_opened_sxpfvID) == 2 ||
        MinorDevID_To_ValidChannelNr(&SXPFv_Info.current_opened_sxpfvID) == 3 ) && 
      SXPFv_Info.cuurent_opened_real_sxpf_Nr == 1) { 
      pr_info("SXPFv: now, i am in ioctl_send_cmd_sync_v");
      int ret;
      ret = sxpf_mapping_option(mode, arg);
      if (ret) {
        return ret;
     ...
     } else{ pr_info("SXPFv: now, in synch else,  port");  }
 ...
 * SXPFv_Info.channels_per_device[j] (hier is aquivalent to SXPFv_Info.channels_per_device[0]
 *   + SXPFv_Info.channels_per_device[1], i.e. 4 + 2 = 6), summarize this array and then
...
static int sxpfv_create(int dev_num_sum, int total_num_of_channels) {
  ...
  for (j = 0; j < dev_num_sum; j++) {
    for (i = 0; i < SXPFv_Info.channels_per_device[j]; i++) {
      if (!SXPFv_Info.channels_per_device[j]) {
        pr_info("class creation finish\n");
        // break;
      } else {
        if (j > 0) {
          k = SXPFv_Info.channels_per_device[j - 1] +
...
static int __init sxpfv_init(void) {
  ...
  for (j = 0; j < nr_dev; j++) {
    /**
     * In this kernel module this function get_num_of_channels(int) is not actually used(for replay card is only has 4 channels),
     * but in order to meet some configuration of K8s, card 1 and card 0 are all set the number channels into 8
    */
    temp = get_num_of_channels(j); 
    temp = 8;  // Always use the maximal number of channel
    SXPFv_Info.total_channels += temp;
    SXPFv_Info.channels_per_device[j] = temp;  // Count the channel number of every device
  }

  pr_info("Debug: nr_dev:%d", nr_dev);
  pr_info("Debug: total number of channels: %d", SXPFv_Info.total_channels);

  sxpfv_create(nr_dev, SXPFv_Info.total_channels);
...
static void __exit sxpfv_exit(void)  // Unregister the device when unload the kernel module
{
  ...
  for (i = 0; i < SXPFv_Info.total_channels; i++) {
  ...
  kfree(virtual_device_structures);
  unregister_chrdev_region(dev, SXPFv_Info.total_channels);  // Release the devcie number
...
static int sxpfv_open(struct inode *inode, struct file *file) {
  ...
  do {
    channel_acount += SXPFv_Info.channels_per_device[j];
    j++;
  ...
  if (IS_ERR(f)) {
    ...
    struct cdev *cdev = f->f_inode->i_cdev;
    sxpf_card_t *card = container_of(cdev, sxpf_card_t, cdev);
    sxpf_file_hdl_t *fhl = container_of(&card, sxpf_file_hdl_t, card);

    SXPFv_Info.cuurent_opened_real_sxpf_Nr = fhl->card->nr;
    SXPFv_Info.current_opened_sxpfvID = inode->i_rdev; 
    pr_info("SXPFv: sxpf nr of device:%d", fhl->card->nr);
...
static int sxpfv_release(struct inode *inode, struct file *file) {
  ...
  if (ret) {
    ...
    pr_info("SXPFx: closed device sxpfv\n");
    return 0;
  }
}

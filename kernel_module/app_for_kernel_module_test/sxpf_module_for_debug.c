...
#endif
{
    ...
    case IOCTL_SXPF_READ_CONFIG: {
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_READ_CONFIG\n");
        ret = ioctl_read_config(card, arg);
    } break;

    case IOCTL_SXPF_WRITE_CONFIG:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_WRITE_CONFIG\n");
        ret = ioctl_write_config(card, arg);
        } break;
        

    case IOCTL_SXPF_GET_CARD_PROPS:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_GET_CARD_PROPS\n");
        ret = copy_to_user((void __user *)arg, &card->props, sizeof(card->props));
        } break;
        

    case IOCTL_SXPF_RELEASE_FRAME:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_RELEASE_FRAME\n");
        ret = ioctl_release_frame(fh, arg);
        } break;
        

    case IOCTL_SXPF_START_RECORD:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_START_RECORD\n");
        ret = ioctl_start_record(fh, arg);
        } break;
        

    case IOCTL_SXPF_START_PLAYBACK:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_START_PLAYBACK\n");
        ret = ioctl_start_playback(fh, arg);
        } break;
        

    case IOCTL_SXPF_ALLOC_PLAY_FRAME:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_ALLOC_PLAY_FRAME\n");
        ret = ioctl_alloc_play_frame(fh, arg);
        } break;
        

    case IOCTL_SXPF_STOP:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_STOP\n");
        ret = sxpf_stream_stop(fh, arg);
        } break;
         

    case IOCTL_SXPF_READ_REG:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_READ_REG\n");
        ret = ioctl_read_register(card, arg);
        } break;
        

    case IOCTL_SXPF_WRITE_REG:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_WRITE_REG\n");
        ret = ioctl_write_register(card, arg);
        } break;
        

    case IOCTL_SXPF_CMD_SYNC:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_CMD_SYNC\n");
        ret = ioctl_send_cmd_sync(fh, arg);
        } break;
        

    case IOCTL_SXPF_TRIG_EXPOSURE:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_TRIG_EXPOSURE\n");
        ret = ioctl_trigger_exposure(card, arg);
        } break;
        

    case IOCTL_SXPF_GET_TIMESTAMP:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_GET_TIMESTAMP\n");
        ret = ioctl_get_timestamp(card, arg, 0);
        } break;
        

    case IOCTL_SXPF_GET_TIMESTAMP_ALT:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_GET_TIMESTAMP_ALT\n");
        ret = ioctl_get_timestamp_fast(card, arg, 0);
        } break;
        

    case IOCTL_SXPF_GET_TIMESTAMP_SEC:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_GET_TIMESTAMP_SEC\n");
        ret = ioctl_get_timestamp(card, arg, 1);
        } break;
        

    case IOCTL_SXPF_TIMESTAMP_SYNC:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_TIMESTAMP_SYNC\n");
        ret = ioctl_timestamp_sync(card, arg);
        } break;
        

    case IOCTL_SXPF_SET_USR_SEQUENCE:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_SET_USR_SEQUENCE\n");
        ret = ioctl_set_user_sequence(card, arg);
        } break;
        

    case IOCTL_SXPF_ENABLE_USER_SEQ:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_ENABLE_USER_SEQ\n");
        ret = ioctl_enable_user_sequence(card, arg);
        } break;
        

    case IOCTL_SXPF_GET_BUF_HDR:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_GET_BUF_HDR\n");
        ret = ioctl_get_buf_header(card, arg);
        } break;
        

    case IOCTL_SXPF_INIT_USER_BUFFER:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_INIT_USER_BUFFER\n");
        ret = ioctl_init_user_buffer(fh, arg);
        } break;
        

    case IOCTL_SXPF_INIT_I2C_SLAVE:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_INIT_I2C_SLAVE\n");
        ret = ioctl_init_i2c_slave(fh, arg);
        } break;
        

    case IOCTL_SXPF_REMOVE_I2C_SLAVE:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_REMOVE_I2C_SLAVE\n");
        ret = ioctl_remove_i2c_slave(fh, arg);
        } break;
        

    case IOCTL_SXPF_I2C_SLAVE_TX_ACK:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into IOCTL_SXPF_I2C_SLAVE_TX_ACK\n");
        ret = ioctl_i2c_slave_tx_ack(card, arg);
        } break;
        

    default:{
        CARD_PRINTF(SXPF_INFO, "SXPFr: now into default\n");
        ret = -ENOIOCTLCMD;
        } break;
